...
func TestEmitterWritesAConfiguredHeader(t *testing.T) {
	e := NewEmitter()
	e.GeneratedHeader = "// Code generated by Popcorn Wave via TinyBind; DO NOT EDIT."

	source, err := e.Decoder(decoderRoute("/about", "about"), nil)
	if err != nil {
		t.Fatalf("Decoder: %v", err)
	}
	if got := strings.SplitN(string(source), "\n", 2)[0]; got != e.GeneratedHeader {
		t.Errorf("first line = %q, want the configured header", got)
	}
}

func TestGeneratedHeaderPrefixPairsWithTheHeader(t *testing.T) {
	e := NewEmitter()
	// The default has to come back as the prefix discovery already recognizes,
	// or the pairing would silently register the wrong string.
	if got := e.GeneratedHeaderPrefix(); got != gensource.DefaultPrefix {
		t.Errorf("default prefix = %q, want %q", got, gensource.DefaultPrefix)
	}

	e.GeneratedHeader = "// Code generated by Popcorn Wave via TinyBind; DO NOT EDIT."
	if got := e.GeneratedHeaderPrefix(); got != "Code generated by Popcorn Wave via TinyBind" {
		t.Errorf("branded prefix = %q", got)
	}

	// A header a framework wrote without the conventional ending still yields a
	// prefix, because the recognition check is what enforces the ending.
	e.GeneratedHeader = "// Popcorn Wave output"
	if got := e.GeneratedHeaderPrefix(); got != "Popcorn Wave output" {
