...
single:
  Load: "func Load[T any, PT interface{*T; ItemDecoder}](ctx, table string, key dynamodb.Key, opts ...dynamodb.GetOption) (T, error)"
  Store: "func Store[T ItemEncoder](ctx, table string, v T, opts ...dynamodb.WriteOption) error"
  Remove: "func Remove[T Keyer](ctx, table string, v T, opts ...dynamodb.WriteOption) error"
  Update: "func Update[T Keyer](ctx, table string, v T, update string, opts ...dynamodb.WriteOption) error"
returning:
  StoreReturning: "func StoreReturning[T ItemEncoder, PT interface{*T; ItemDecoder}](ctx, table string, v T, opts ...dynamodb.WriteOption) (T, bool, error)"
  RemoveReturning: "func RemoveReturning[T Keyer, PT interface{*T; ItemDecoder}](ctx, table string, v T, opts ...dynamodb.WriteOption) (T, bool, error)"
...
paged:
  QueryPage: "func QueryPage[T any, PT ...](ctx, table, keyCond string, opts ...dynamodb.QueryOption) (Page[T], error)"
  ScanPage: "func ScanPage[T any, PT ...](ctx, table string, opts ...dynamodb.ScanOption) (Page[T], error)"
...
iterated:
  Query: "func Query[T any, PT ...](ctx, table, keyCond string, opts ...dynamodb.QueryOption) iter.Seq2[T, error]"
  Scan: "func Scan[T any, PT ...](ctx, table string, opts ...dynamodb.ScanOption) iter.Seq2[T, error]"
...
context_client:
  ...
  parameter_twin: each entry here is also a method on Handle, per requirement:dynamo-parameter-api and decision:generic-method-migration; the entries named above are the Context form and stay the default, and the "On" functions of the earlier spelling are gone
  twin_is_discovered: calling either spelling emits the same codec, per requirement:parameter-api-call-discovery
