...
pruned_2026_08_14:
  what: WriteJSON and the one-line jsonbind delegations (RawJSONMap, BytesJSONMap, RawJSONArray, DecodeJSONMapStringString, the DecodeJSON scalar and slice family) removed from httpbind and fasthttpbind together
  why: generated code has called jsonbind directly since the codec rewrite (827f21f), and no caller in this module or downstream ever used the per-surface spellings; WriteJSON was also dropped from the transform's transport-only pattern list
  parity: removed pair-wise, so the two runtime surfaces still mirror each other; the "each surface spells its own" finding under done_2026_08_08 no longer applies to the codec helpers
```
