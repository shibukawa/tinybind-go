...
go_signature:
  ...
  optional_context:
    ...
    caller_scope: the detection reaches the compiler as a caller-supplied map, so it holds only where the caller fills it; a route package does not, which requirement:route-package-context-externals covers
usage:
