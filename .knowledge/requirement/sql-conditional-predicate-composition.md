...
status: complete 2026-08-17; every clause in scope, with CASE reported rather than elided
...
scope:
  ...
  comma_clauses: SET, VALUES, ORDER BY, GROUP BY, FROM, WITH, WINDOW, USING, and PARTITION BY, plus the INSERT column list; SELECT and RETURNING stay text because a conditional item there is refused outright
  case_expressions: reported rather than elided, per rule:sql-predicate-group-elision exactness
