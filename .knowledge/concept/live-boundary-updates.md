...
problem:
  ...
  client_driven_only: the one later update in the model is requirement:component-redraw-endpoint, which the browser must ask for, so a server that learns something new cannot push a new render of a region it already sent
