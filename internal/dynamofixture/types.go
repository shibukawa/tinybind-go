...
func Fetch(ctx context.Context, c *dynamodb.Client, table string, key dynamodb.Key) (Reading, error) {
	return dynamobind.Load[Reading](ctx, c, table, key)
}

// Save stores one reading, replacing whatever shared its key.
func Save(ctx context.Context, c *dynamodb.Client, table string, r Reading) error {
	return dynamobind.Store(ctx, c, table, r)
}

// Replace stores one reading and returns the one it replaced, if any.
func Replace(ctx context.Context, c *dynamodb.Client, table string, r Reading) (Reading, bool, error) {
	return dynamobind.StoreReturning(ctx, c, table, r)
}

// Delete removes the reading sharing r's key.
func Delete(ctx context.Context, c *dynamodb.Client, table string, r Reading) error {
	return dynamobind.Remove(ctx, c, table, r)
}

// Retire deletes the reading sharing r's key and returns what was deleted.
func Retire(ctx context.Context, c *dynamodb.Client, table string, r Reading) (Reading, bool, error) {
	return dynamobind.RemoveReturning(ctx, c, table, r)
}

// Correct applies an update expression to the reading sharing r's key.
func Correct(ctx context.Context, c *dynamodb.Client, table string, r Reading, update string, opts ...dynamodb.WriteOption) error {
	return dynamobind.Update(ctx, c, table, r, update, opts...)
}

// Page reads one page of a sensor's readings.
func Page(ctx context.Context, c *dynamodb.Client, table, keyCond string, opts ...dynamodb.QueryOption) (dynamobind.Page[Reading], error) {
	return dynamobind.QueryPage[Reading](ctx, c, table, keyCond, opts...)
}

// Each iterates every reading a query matches, one request per page.
func Each(ctx context.Context, c *dynamodb.Client, table, keyCond string, opts ...dynamodb.QueryOption) iter.Seq2[Reading, error] {
	return dynamobind.Query[Reading](ctx, c, table, keyCond, opts...)
}

// Sweep iterates a whole table.
func Sweep(ctx context.Context, c *dynamodb.Client, table string, opts ...dynamodb.ScanOption) iter.Seq2[Reading, error] {
	return dynamobind.Scan[Reading](ctx, c, table, opts...)
}

// SaveAll stores every reading, in requests DynamoDB accepts.
func SaveAll(ctx context.Context, c *dynamodb.Client, table string, rs []Reading) ([]Reading, error) {
	return dynamobind.StoreAll(ctx, c, table, rs)
}

// FetchAll reads every key, in requests DynamoDB accepts.
func FetchAll(ctx context.Context, c *dynamodb.Client, table string, keys []dynamodb.Key) ([]Reading, []dynamodb.Key, error) {
	return dynamobind.LoadAll[Reading](ctx, c, table, keys)
}
