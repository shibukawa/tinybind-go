...
// tinybind:generated v1 inputs=sha256:ed74d7c96f94e9b4b26487cd2f077fb384b9d3dab2598624d3144eb59f4756c6 self=sha256:b0c26b0c4147526932081f9f8f12ea1f33a6cf677b485e49d22c232c202d3635 outputs=tinybind_gen.go,configbind_gen.go,tinybind_openapi_gen.go
...
func registerWebServerConfigDefinition0() {
	configbind.Register[WebServerConfig](configbind.Definition{
		...
		DependsOn: map[string][]configbind.Dependency{
			"webserver.tracing_url":   {{Key: "webserver.tracing"}},
			"webserver.tls.cert_path": {{Key: "webserver.tls.enabled"}},
		},
		...
		FlagMetas: []cliparser.FieldMeta{
			...
			{Prefix: "webserver", Key: "protocols", Help: "protocols the listener offers", Enum: []string{"http1", "http2", "http3"}, Kind: cliparser.KindArray},
			...
			{Prefix: "webserver", Key: "tracing", Help: "tracing exporter", Enum: []string{"off", "otlp", "jaeger"}},
		...
		Scaffold: []configbind.ScaffoldField{
			...
			{Key: "protocols", Kind: configbind.ScaffoldStringSlice, Help: "protocols the listener offers", Enum: []string{"http1", "http2", "http3"}},
			...
			{Key: "tracing", Kind: configbind.ScaffoldString, Help: "tracing exporter", Enum: []string{"off", "otlp", "jaeger"}},
			...
			{Key: "routes", Kind: configbind.ScaffoldTableArray, Help: "static routes, one [[webserver.routes]] table each", Nested: []configbind.ScaffoldField{
				...
				{Key: "kind", Kind: configbind.ScaffoldString, Help: "how the route is served", Enum: []string{"static", "proxy"}},
...
func applyWebServerConfigDefinition0(dst any, o *configbind.Overlay) error {
	...
	if v, ok := o.GetMulti("webserver.protocols"); ok {
		for _, item := range v {
			switch item {
			case "http1", "http2", "http3":
			default:
				return fmt.Errorf("configbind: webserver.protocols: %q must be one of: http1, http2, http3", item)
			}
		}
		p.Protocols = v
	}
	...
	if v, ok := o.GetString("webserver.tracing"); ok {
		switch v {
		case "off", "otlp", "jaeger":
		default:
			return fmt.Errorf("configbind: webserver.tracing: %q must be one of: off, otlp, jaeger", v)
		}
		p.Tracing = v
	}
	...
	if ta1, ok := o.Get("webserver.routes"); ok {
		...
		for i1 := range ta1.Tables {
			...
			if v, ok := ta1.Tables[i1].GetString("kind"); ok {
				...
				case "static", "proxy":
				default:
					return fmt.Errorf("configbind: webserver.routes[%d].kind: %q must be one of: static, proxy", i1, v)
				}
				p.Routes[i1].Kind = v
			}
