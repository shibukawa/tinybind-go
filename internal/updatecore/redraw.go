...
func (o Options) Redraw(r Reader, reg *Registry, options ...htmlbind.Option) (Response, bool) {
	...
	instance := r.Header(o.InstanceHeader())
	if kind == "" || instance == "" {
...
func (o Options) redrawResponse(r Reader, component Reloadable, kind, instance string, options []htmlbind.Option) Response {
	if len(r.RawQuery()) > o.QueryLimit() {
		return o.Refuse(r, Failure{
	...
	fragment, err := component.Render(r.Context(), instance, r.Query())
	if err != nil {
		return o.Refuse(r, Failure{
...
// MatchesETag reports whether an If-None-Match header names etag. It is
// exported because each shell answers a conditional request for the runtime
// asset itself, and the comparison has to be the one the entries use.
