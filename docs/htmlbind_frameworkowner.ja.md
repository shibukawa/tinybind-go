...
最後の行が意外に思われる点です。`htmlbind.Content` は境界 ID とレンダリング済み
HTML しか持ちません。これは意図的で、ストリーミング中のドキュメントにも、JSON
ペイロードにも、任意の形に載せられるようにするためです。
...
**推移的です。** 生成時にコンポーネント呼び出しグラフを辿るので、自身が `await`
を宣言していなくても、async なコンポーネントを呼ぶだけで `true` になります。

**何もレンダリングしません。** フラグは生成されたプラン上の定数で、束縛値に写さ
れるだけです。読んでも goroutine は起きず、シーケンスも消費しないので、エントリ
ポイントを選ぶ前に聞けます。

**渡した Fragment は見えません。** パラメータ構造体を経由してコンポーネントに渡
...
寄与は外側から順にマージされ、後から来た重複は落ちます。2つのコンポーネントが同
じスタイルシートを宣言してもタグは1つです。同一性はマージ済み文字列の完全一致で
す。

制約を1つ把握しておいてください。`Bind` はプラン自身の寄与しか写さないので、パラ
メータ構造体を経由してコンポーネントに渡された `Fragment` は、その head 寄与をマー
ジ済みドキュメント head に届けません。テンプレート内で静的に呼ばれるコンポーネン
トは問題ありません — コンパイラが呼び出し側のプランに畳み込みます。穴は、実行時に
...
なお、コンポーネントのスタイルとスクリプトは現在インライン markup としてドキュメ
