...
| live レスポンスをどこまで開いたままにするか、いつ閉じるか | 実装者 |
| builtin element の provider を「発行」ではなく「セッション状態の読み取り」にすること | 実装者 |
| CSRF トークンの生成・保管・破棄と、その検証 | 実装者 |
| 再描画の head 寄与をドキュメントシェルに入れておくこと | 実装者 |
...
最後の3行は新しく、いずれも**モジュールが仕組みを用意して方針は意図的に持たなかっ
た**場所です。下の[実装者が自分でやること](#実装者が自分でやること)にまとめてありま
す。どれも、配線するまで静かに何もしないためです。
...
component のスタイルとインラインスクリプトは、`PublicURLBase` の下に内容ハッシュ付
きのファイルとして抽出され、head にはインラインのブロックではなく参照タグが載りま
す。インラインスタイルを禁止するポリシーでも動きます。ビルドが書き出すのは
`Result.Assets` です。`htmlbind.MergeAssets(wrappers, leaf)` は同じ集合を識別子とし
て返し、描画を始める前に束縛済みの値から読めます。ドキュメントシェルが何を先に持っ
ておくべきかを決めるためのものです。

### builtin element

フレームワークは、作者が名前で書くマークアップを登録できます。
...
作者は生成単位のどのテンプレートでも `<csrf-token />` と書けます。接頭辞も import
もファイルごとの宣言も要りません。

これが external 呼び出しの糖衣ではなく独立した継ぎ目である理由は、**起きないこと**
のほうにあります。**値がテンプレートのスコープに入りません。** 名前が束縛されないの
で、別の場所に埋め込むことも、クエリ文字列に入れることも、ログに出すこともできませ
ん。同じトークンを返す external はそれをテンプレートに手渡すので、テンプレートが値
に対してできることはすべてできるようになります。両方の継ぎ目があり、どちらも他方を
置き換えません。

生成が element を書き換えます。マークアップの固定部分は周囲の静的な連なりに畳み込
まれるので、作者が直接書いた場合と同じコストです。残るのは `TokenFor(ctx)` を呼ん
で各ホールを書く1つのプランステップだけです。provider も式の属性も持たない定義は、
まるごと静的バイト列に還元され、ステップを1つも増やしません。

各ホールは位置に応じてエスケープされ、ホールが置けるのは要素テキストと引用符付きの
属性値だけです。引用符を含むトークンは、自分が入っている属性から抜け出せません。フ
レームワークが同じマークアップを信頼済み文字列として組み立てた場合に失われるのが、
まさにこの性質です。

provider を宣言すると3つのことが従います。3つとも宣言ではなく導出なので、登録内容
が自分自身と食い違うことはありません。

- **リクエストごとの値になる。** これに到達する component は出力を保存できません。
  保存されたボディはある訪問者のトークンを次の訪問者に配ることになり、これは古い値
  のバグではなくセキュリティの失敗です。この排除は呼び出しグラフをたどります。拒否
  するのは `ttl` であって注釈そのものではないので、スコープの宣言はできます。
- **context が要る。** context なしの描画は element を名指しして失敗します。値の不
  在をそのまま描くことはしません。
- **provider は失敗しうる。** 初回パスの中であればレスポンスが確定する前なので、エ
  ラーステータスを選ぶ余地が残ります。

宣言が必要なのは `Vary` だけです。provider が何を読むかを知っているのは実装だけだか
らです。これは束縛済みの値に `Fragment.Vary()` と
`htmlbind.MergeVary(wrappers, leaf)` として届きます。cookie に依存するレスポンスは
他のどこでもそれを名乗らず、何も報告しない依存から `Vary` ヘッダもキャッシュキーも
組み立てられません。

provider のシグネチャは `func(context.Context) (V, error)` です。ここでは検査しませ
ん。context を取る external と同じで、自分の Go ソースを読むのは呼び出し側で、不一
致を弾くのは Go コンパイラです。`Result` が `V` を名指しするのは、ホールのクロージャ
を書き下す必要があり、Go は呼び出しの型引数は推論しても関数リテラルの引数型は推論し
ないからです。

### ハイフン付き要素の空間は閉じている

ハイフンは HTML 自身のカスタム要素の目印なので、builtin を登録するということは、
**それ以外の**ハイフン付き要素をどう扱うかを決めるということです。答えは、この空間
をホワイトリストにすることです。載っているものは宣言済みで、それ以外は生成エラーに
なり、ファイル・行・桁を名指しします。

これが接頭辞で解決しようとしていたケースを消します。今日の `<csrf-toekn />` はブラ
ウザが無視し、誰も報告しないマークアップです。空間を閉じればコンパイルエラーになり、
`<csrf-token>` を提案します。

アプリケーションの Web Components も同じホワイトリストに、名前または接頭辞グロブで
入ります。

```go
options.PassthroughElements = []htmlbind.PassthroughElement{
	{Name: "sl-*"},        // コンポーネントライブラリまるごと、宣言は1回
	{Name: "my-widget"},
}
```

passthrough の要素はそのまま出力され、プランステップを作りません。builtin の名前は、
それを覆うグロブより常に優先されます。逆は起きません。`<svg>` と `<math>` の中のハ
イフン付きの名前は標準の外部名前空間の要素なので、ホワイトリストの外にあります。

**既存プロジェクトにとっての唯一の挙動変更がこれです。** 既に Web Components を書い
ているプロジェクトは passthrough エントリを一度追加するだけで、診断が宣言すべき要素
をすべて名指しします。書いていないプロジェクトは影響を受けず、バイト単位で同じもの
を再生成します。

設計のうち2つは未実装です。**opaque シェイプ**（信頼済みの値や fragment を返す
provider で、値だけでなく*構造*が変わる出力のためのもの）は先送りです。信頼の主張が
フレームワーク側のコードに移り、生成側が出力の構造を検証できなくなるという代償があ
るためです。もう1つ、**`<head>` 宣言の中の** builtin element は今も head バリデータ
に拒否されるので、`Placement: PlaceHead` は現状「body では拒否する」を意味し、「head
では受け付ける」はまだ意味しません。
...
以下はすべて、モジュールが用意した仕組みと、モジュールが持たない方針の組です。どれ
も配線しなくても動き、どれも配線するまで少しずつ間違ったことをします。ここにまとめ
たのは、**静かに劣化する継ぎ目は失敗する継ぎ目より厄介**で、以下のどれも失敗しない
からです。

### live レスポンスをどこまで開いたままにするか決める

モジュールは健全な live レスポンスを自分から閉じません。購読がどれだけ生きてよいか、
1セッションがいくつ持ってよいか、境界がどれだけ無音でよいか — これらは配信側だけが
知っている予算です。

モジュールが渡すのは「この close は健全だ」と言うレコードのほうです。
...
`retry` を読んだクライアントは速やかに再接続し、**試行回数を消費しません**。障害と
読んだ close は指数バックオフに入ります。つまり上限のないレスポンスは単に長生きなだ
けではなく、**認可の再チェックも、新デプロイへの乗り換えも、再バランスも永遠に起き
ません**。それらは寿命の上限が買うものだからです。

モジュール自身が面倒を見る唯一のケースは自分の終わり方です。ctx のキャンセル — シャッ
トダウン、ローリングデプロイ、こちらが張った期限 — では `done` ではなく `retry` で閉
じます。これが無いと、デプロイのたびに全ての開いている画面に「ソースは終わった」と
伝えてしまい、誰かがリロードするまで凍ります。

`retryMs` は実装者が埋めるもので、実装者にしか埋められません。クライアントのバック
オフは障害にしか反応できません。負荷を落としている最中か、デプロイの最中かを知って
いるのは実装者だけで、**何かが壊れる前に**戻りを散らせるのも実装者だけです。

### provider を「発行」ではなく「読み取り」にする

**provider は同じセッションに対して同じ値を返さなければなりません。** 1回呼んでも2回
呼んでも呼ばなくても、答えは1つである必要があります。通常の形はミドルウェアが値を
context に入れ、provider が取り出すだけ — 往復ではなく map 参照1回です。

これは性能上の注意ではなく要件です。理由は**2つ目の経路**にあります。CSRF トークンは
ブラウザに2回届きます。スクリプトが読むためのレスポンスヘッダと、スクリプト無しでも
フォームが動くための hidden input です。**ヘッダは1つの値しか運べません。**1ページ
に2つのフォームがあって別々のトークンを持っていると、それは片方が送信されるまで誰も
観測できないバグで、観測されたときには診断の付かない送信失敗になります。

そこでモジュールは自分の側の約束を果たします。**provider は1描画につき最大1回しか呼
ばれず、すべての出現が結果を共有します。**メモのキーは要素ではなく **provider** なの
で、hidden input と meta タグが同じ関数に支えられている場合も食い違いません。

スコープは1描画です。再描画と live 配信は呼び直します。これが正しいのは、どちらも自
分のヘッダを持つ別のレスポンスだからです。失敗はメモしません。描画が終わるので共有
する相手がいないからです。

呼び出しごとに新しい値を発行する provider はこれを壊し、しかも検出できません。モ
ジュールには安定した読み取りと不安定なそれの区別がつかず、症状は描画ではなくフォー
ム送信の側に出ます。

### CSRF トークンのライフサイクルを持つ

このモジュールが描画する unsafe form はすべて hidden field を持ち、ランタイムは自分
が出すすべてのリクエストに同じ値をヘッダで載せます。**作者は何も書きません。**それが
目的です。自分で書くことを覚えていなければならないセキュリティ制御は、いつか忘れる
もので、忘れた結果は「動くページ」であり、失敗するのは送信のときだけです。
...
context から読むのではなく描画オプションなのは、**このモジュールが context から読め
ない**からです。キーはセッションを持つ側のもので、こちらには引くべきものがありませ
ん。自分の描画入口で1回渡せば、ハンドラ側は何も変わりません。

生成はログイン時またはセッション作成時、保管はセッションごとに1個、破棄はログアウト
時とセッション再生成時。**リクエストごとにローテーションしないでください。**2つ目の
タブのフォームは、1つ目のタブの送信が既に置き換えたトークンを持つことになり、その代
償に得るものがありません。セッションごとに1個であることが、ヘッダと hidden field を
一致させている条件でもあります。ヘッダは1つの値しか運べません。

セッションを持たない描画 — メール本文、静的エクスポート、ゴールデンテスト — は
`WithoutCSRFToken()` と明示します。明示的なのは、「トークンが無い＝要らない」と解釈
してしまうと、**オプションの書き忘れが「送信して拒否されるフォーム」に化け、原因を指
すものが何も残らない**からです。

**Origin と Fetch Metadata の検証は実装者のもので、やる価値があります。**このモジュー
ルはリクエストの出所を一切見ません。描画より前の受信リクエストに対する検査なのでミド
ルウェアの領分で、ラップするものは Go の `http.CrossOriginProtection` です。両方走ら
せてください。**2つは無関係な理由で壊れます。**`Origin` を書き換えるプロキシや、
credentials 付き CORS の設定ミスは、オリジン系防御を一斉に、しかも失敗するリクエスト
を1つも出さずに無効化します。トークンはその故障を共有しません。加えて兄弟サブドメイ
ンは SameSite にとっても `Sec-Fetch-Site` にとっても `same-site` なので、`cross-site`
だけを拒否するルールは素通しします。

オリジン検査だけで行くと決めたなら、`CSRFMode: CSRFOff` が field・ヘッダ・per-request
マーキングをまとめて切ります。トークンが課している唯一のコストが戻ってきます。

### キャッシュするリストと、キャッシュしないフォームを分ける

**unsafe form を描画する component は出力を保存できません。**保存されたボディはある
セッションのトークンを次に来た人へ配ることになり、これは古い値のバグではなくセキュリ
ティの失敗です。生成エラーになり、呼び出しグラフを辿ります。拒否されるのは `ttl` で
あって注釈そのものではないので、`@cache(scope: ...)` による宣言はできます。

これが押し付ける構成は、もともと正しい構成でもあります。DB から取ったリストはキャッ
シュし、フォームはキャッシュせず、2つの component に分ける。

```
@cache(ttl: "1m") component List(rows: string[])   ← キャッシュ可
component Form()                                   ← トークンを持つ
export component Page(rows: string[])              ← 両方を組む
```

### 再描画の head をドキュメントシェルに入れておく

再描画は、そのエンドポイントが描画したわけではないページにマークアップを差し込むの
で、自分が持つ head にマージできません。レスポンスは `X-Tinybind-Head` を運び、ラン
タイムが差し替え前に足りないものを設置します。ただしこれは**修復であって計画ではあ
りません**。計画のほうは、タグが最初からそこにあることです。

```go
shell := layout.LayoutParams{Head: registry.RequiredHead()}
```

これをやっておくと、ヘッダは毎回の再描画で何も設置しません。名指しされたものが全部
既にあるからです。やらないと、各 component の初回再描画が差し替えの途中でスタイル
シートの取得を払います。**壊れるのではなく遅くなるだけ**なので、長いあいだ気づかず
に放置できてしまう類の項目です。

### fragment レスポンスが何を負うか決める

`Fragment.Head()` と `Fragment.Assets()` は、その値が必要とするものを報告します。ス
ロットで渡された component が必要とするものも含みます。ドキュメントシェルを持たない
レスポンスには、どちらも置き場所がありません。それが何を意味するかをモジュールは決
めません。黙って捨てるのも、レスポンスを拒否するのも、どちらも筋が通っていて、フレー
ムワークがどちらを約束しているかを知っているのは実装者だけです。
