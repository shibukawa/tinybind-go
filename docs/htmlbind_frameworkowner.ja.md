...
- **リクエストごとの値になる。** これに到達する component は `@cache` できません。
  保存されたボディはある訪問者のトークンを次の訪問者に配ることになり、これは古い値
  のバグではなくセキュリティの失敗です。この排除は呼び出しグラフをたどります。
...
**unsafe form を描画する component は `@cache` できません。**保存されたボディはある
セッションのトークンを次に来た人へ配ることになり、これは古い値のバグではなくセキュリ
ティの失敗です。生成エラーになり、呼び出しグラフを辿ります。
