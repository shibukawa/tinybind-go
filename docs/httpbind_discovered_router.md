...
The key is the header rather than the file name, so redirecting the output to a
name of your own changes nothing.

Replacing the header is the case that needs one more line. If you brand your
generated files, tell discovery the prefix, or the registry you just wrote is
analyzed as if a user had written it and its page registrations become documented
routes:
...
`GeneratedHeaderPrefix` is the pairing, so the registered string cannot drift from
the header. A registered prefix still has to end in the conventional `DO NOT
EDIT.`, and another tool's generated code stays discoverable by default — those
registrations are your application's own API surface, and dropping them would
remove routes from a document with nothing said about it.
...
e.Symbols.WriteError = "WriteProblem"   // the failure entry a handler writes through
```
