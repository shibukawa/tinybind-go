...
意図しない呼び出しにならないよう、規則が 3 つあります。`{val}` は、その位置で既に
見えている名前を使えません——先行する束縛、外側の束縛、パラメータのいずれでも。1 つの
`{val}` の束縛どうしは独立なので、片方がもう片方を読むなら 2 つに分けてください。
そしてどこからも読まれない束縛はエラーです——statement を組み立てるたびに呼ばれて、
結果はどこにも行きません。
...
末尾の error は、生成側がパッケージの Go ソースを読んで検出します。generator コマンド
