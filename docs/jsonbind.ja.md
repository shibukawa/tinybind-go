...
- `bool`
- `float64`
- `byte` と `rune`(それぞれ `uint8`、`int32` そのものです)
- scalar 上の名前付き型(フィールドとしても、コレクションの要素としても)
- 上記の slice
- `[4]int` や `[2]Address` などの固定長配列
- `[]byte` と `[N]byte`(base64)
- ネストした構造体
- 構造体の slice
...
`[N]T` のフィールドは、slice のように作り直して代入するのではなく、その場を埋めま
す。宣言した長さの両側は対称ではありません。

- **要素が足りないときは、届いた分だけ埋めて残りはゼロ値のままにします。** 長さは
  Go の型がすでに述べていることであって、ドキュメントが毎回言い直す必要はありませ
  ん。`[4]int` に `{"cells": [7]}` が届けば `[7 0 0 0]` になります。
- **要素が多すぎるときはエラーです。** 先頭の `N` 個だけ取れば末尾は落ちます。それ
  は宣言した長さがまさに防ぐための、黙って起きるデータの欠落です。エラーはメンバー
  名を挙げ、原因として `jsonbind.ErrArrayTooLong` を返します。HTTP 越しなら 400 で
  す。

`null` のメンバーはフィールドに触れません。slice と同じです。同じメンバーが2回届い
たときは、2つが重なった配列ではなく2つ目の配列になります。

エンコード側にこうした取り決めは要りません。`[4]int{1, 2, 0, 0}` は毎回 `[1,2,0,0]`
と、長さぶんそのまま書き出します。配列には潰すべき nil の形がないからです。

slice でできて配列で拒否することが1つあります。構造体の配列への `omitzero` です。
Go の配列は要素型の比較可能性をそのまま引き継ぐためで、黙って取りこぼすのではなく生
成時のエラーにします。

### 名前付き型

scalar 上の名前付き型は、ワイヤ上ではその scalar そのものです。`encoding/json` の挙
動と同じで、codec が両方向で変換します。
...
コレクションでも同じです。宣言した型は往復しても失われないので、`Guests` は
`[]string` ではなく `[]UserID` として返ってきます。slice や map の上の名前付き型 —
`type Tags []string` — は別物で、こちらは今も拒否します。名前を持てるのは要素の側
だけです。

### バイト列

`[]byte` と `[N]byte` のフィールドは base64 文字列になります。`encoding/json` も
`encoding/json/v2` もそう書くからです。`[]uint8` は Go の型として同じものなので、同
じ文字列になります。`map[string]byte` は列ではないので数値の map のまま、単独の
`byte` や `rune` のフィールドも数値のままです。
...
固定長のバイト列は、上の配列の規則をバイト単位で引き継ぎます。長さに足りなければ残
りはゼロのまま、超えていればメンバー名を挙げたエラーです。CBOR ではどちらの綴りも
byte string になります。1要素あたり1バイトではなく、全体で1バイトのオーバーヘッド
で済みます。

`encoding/json` と異なる点が2つありますが、どちらも意図したものです。次節を参照して
ください。HTTP 越しならバイト列は query・path・header・cookie からも届きます。
[httpbind](httpbind.ja.md) を参照してください。
...
生成 codec はドキュメントを前方に一度だけ走査して読み、書き出しはバッファへの append で行います。中間マップを組み立てることも、構造体を reflect で覗くこともありません。出力を `encoding/json` と突き合わせる前に、次の5点を把握しておいてください。
...
- **nil のスライスは `[]`、nil のマップは `{}`、nil の `[]byte` は `""` で出力します**。`encoding/json` はいずれにも `null` を書くので、Go の型が引いていない線引きがクライアントに渡ります。Go 側で「項目がない」と「空のリスト」が区別されない以上、ワイヤ上でも区別されるべきではありません。`encoding/json/v2` は空の配列と空のオブジェクトを書きます。この codec も同じです。
- **`[N]byte` は数値の配列ではなく base64 です**。`encoding/json` が base64 にするのは slice だけで、byte 配列に文字列を読み戻すことはできません。固定長のバイト列 — ハッシュ、タグ、UUID — こそ base64 が欲しい場面なので、ここでは両方の綴りを同じに扱います。
