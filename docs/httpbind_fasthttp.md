...
## Page trees on fasthttp

A filesystem route tree is generated by `routetree` rather than by the transform
above, because generated files are outputs and not transform inputs. So the
emitter picks the transport, and one call sets every part of it:
...
files, err := routetree.Generate(routetree.GenerateOptions{
    ...
    RootPackage: "pages",
    Emitter:     routetree.NewFastHTTPEmitter(""), // "" is the tinygodriver fork
})
```

That changes three things at once, which is why it is one call rather than three
settings: the decoder takes `*fasthttp.RequestCtx` and reads through the runtime
accessors, the registered handler is one parameter instead of two, and a rung 3
`Load` is recognized by the fasthttp signature rather than the net/http one.

The generated `Register` takes a router by interface, because fasthttp ships
none and this module will not choose one for you:
...
Anything with that method satisfies it. There is no `NewServeMux` in the
output, since there is no router for it to build.

By default the patterns reach it exactly as written — Go 1.22 spellings, `{$}`
included. If your router reads a different syntax, say so, because these two
shapes are not rejected by such a router but *misread* by it: `{rest...}` looks
like a parameter named `rest...` and `{$}` like one named `$`, so the route is
silently installed somewhere else.

```go
e := routetree.NewFastHTTPEmitter("")
e.Symbols.CatchAllSuffix = ":*" // {rest...} becomes {rest:*}
e.Symbols.RootPattern = "/"     // instead of /{$}
```

Every other segment is spelled `{name}` by every router this has met, so
nothing else moves. The generated `Routes` table keeps both: `Pattern` is what
was registered, and `Path` is the address the filesystem declared — build a
sitemap from `Path`, because a router's catch-all spelling is not a URL.

One thing worth knowing if you are reaching for the router this module vendors:
`tinygodriver/fasthttprouter` cannot be the `mux` above directly. It declares
`GET`, `POST` and `Handle` rather than a `HandleFunc` taking `"GET /path"`, so a
page tree reaches it through a small adapter — which is where you would apply
the spellings above.

Pass the fasthttp package explicitly if you are on upstream rather than the
fork: `routetree.NewFastHTTPEmitter("github.com/valyala/fasthttp")`.

One behavioural note. On net/http a typed `Load` opening with a
`context.Context` receives `r.Context()`; here it receives the request value
itself, because `*fasthttp.RequestCtx` is a `context.Context`. That context is
cancelled when the server shuts down rather than when the client disconnects —
the same difference the rewrite table records for `r.Context()`.

## What is not done yet

`-backend fasthttp` on the command line drives the transform, not `routetree`.
Page-tree generation is a library API on both transports — there is no flag for
it on net/http either — so call `routetree.Generate` with the emitter above.
