...
そのブロックの終わりまでです。ブロックとは `{if}` の分岐、`{for}` の本体、`{await}`
のサブツリー、component 本体で、**タグはブロックではありません**。`<div>` の中に書いた
束縛も `</div>` の後ろで読めます。値が書ける場所ならどこでも読めます — 補間、属性、
`if` の条件、ループの対象、component の引数、別の external の引数。

値はブロックの先頭で計算されます。束縛をブロックのどこに書いてもです。動くのは計算
だけで、markup の順序も要素の形も変わりません。これがページのローダーを最初の1バイト
より前に失敗させます（[応答を選ぶ](#応答を選ぶ)で使います）。
...
- `{val}` は、その位置で既に見えている名前を使えません — 外側の束縛、パラメータ、
  `{for}` の変数、`{await}` の束縛。**隠せない唯一の束縛です**。動くのがこれだけなので、
  外側の同名を読んでいるものを飛び越すと、その描画結果が変わってしまうからです。
  `{for}` と `{await}` は従来どおり隠せます。
- どこからも読まれない束縛はエラーです。描画のたびに呼ばれて、結果はどこにも行き
  ません。

宣言より前で名前を読むのは今までどおりエラーです。計算は引き上がりますが、名前は
引き上がりません。
...
何も答えない呼び出しがあります。権限のチェック、事前条件の確認 — 欲しいのは描画を
続けてよいかどうかだけで、名前を付ける値がありません。結果型を書かずに宣言して、
`{check}` で呼びます。
...
check は `{val}` から束縛を取り除いたものです。閉じタグはなく、同じブロックの束縛と
同じ位置で評価され、失敗の落ちる先も同じです。制御ブロックの外に書けば、ローダーと
同じようにステータスを選べます。順番は書いたとおりで、束縛より前に書けば先に走るので
拒否されたリクエストはローダーまで届かず、後ろに書けばその名前を読めます。

1 つのディレクティブに 1 呼び出しです。カンマは複数の名前を 1 行に並べるためのもので、
check は共有する名前を持たないので、2 つ目の呼び出しは 2 つ目の `{check}` になります。

結果型のない external にはほかに行き場がなく、書ける場所を名指しで教えます。
...
逆は許されます。`{check LoadData(id)}` は値も返す関数を呼んでその値を捨てます。
ディレクティブが訊いたのは失敗したかどうかだけだからです。非同期版はありません。
境界の失敗はレスポンスが確定した後に落ちるので、描画を拒否するという check の目的を
そこでは果たせないからです。

`@cache` を付けた component の中の check は、キャッシュが蓄えるものの一部です。
ヒットすれば隣のローダーと同じように飛ばされます。宣言済みの parameter だけを読む
check には正しく、リクエストを読む check には誤りで、これはキャッシュされた
component が何に依存してよいかの既存の規則そのものです。

### 応答を選ぶ

ページ自身のローダーがリクエスト全体に答えられます。ステータスヘルパーを返すと、
何も書かれていない状態で描画が止まるので、ステータスはまだ自由です。
...
自身の束縛がチェーンの組み立て中、ドキュメントシェルが何かを書く前に走るからです
——decide・combine・fail のために Go の入口をページに置く必要はありません。

`{if}`・`{for}`・`{await}` の中に書いた束縛はそのブロックが走るときに走るので、
シェルより後です。失敗は描画を終わらせますが、ステータスは選べません。
...
- `html` パラメータを宣言できません。スロット引数は値ではなく束縛された継続であり、
  キーに入れられないためです。
- 直接でも呼び出し先経由でも、`external live` の境界に到達できません。live なソース
  はドキュメントが終わった後も配信し続けるので、保存された範囲が代わりを務められる
  形に settle しないためです。`external async` に対する `await` 境界はちょうど一度
  settle するので、**キャッシュできます**——下記参照。
...
### 自分でデータを読み込む component をキャッシュする

キャッシュ対象の component は `{await}` 境界を含められます。1 つの `@cache` が取得と
描画をまとめて覆えるのはこのためです。
...
初回のリクエストは、store を配っていないときとまったく同じに届きます——プレース
ホルダ、ストリームされる fallback、完了フレーム。store の設定が変えるのは**保存される
もの**であって、**送られるもの**ではありません。キャッシュはデプロイの選択であって、
テンプレートの書き換えではないからです。

保存されるのは placeholder ではなく settle した markup です。2 回目以降はそれがその場
に連続して書かれます——placeholder も fallback も完了フレームもなく、取得もありません。
`await` が同期になった状態です。

失敗した境界は何も保存しないので、次のリクエストはミスになります。settle した
`recover` サブツリーもこの目的では失敗として扱われます。描画されたエラーは、キーが
指す答えではないからです。
...
- `async` パラメータを持つ、live 境界に到達する、あるいは document head を持つ
