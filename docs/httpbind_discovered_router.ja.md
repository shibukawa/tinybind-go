...
- サーバーアクションの中の `httpbind.Bind` はバインダを見つけられない。バインダ
  生成は実装者が書いたルート登録を起点に駆動され、これらは生成コードが登録する
  ので、リクエストモデルが探索されない。その探索がルートパッケージに届くまでは
  標準ライブラリでリクエストを読むこと。これらのエンドポイントが OpenAPI
  ドキュメントに入らないのも同じ境界によるもので、そちらは意図的である ——
  1つの欠落と1つの機能が、同じ原因から出ている。
...
最後の3つは同じ衝突を3度見ているにすぎない。Go の規則とルーティングの慣習が
