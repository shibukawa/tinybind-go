...
| ファイル | 形 | 得られるもの |
| --- | --- | --- |
| `page.tb.html` のみ | テンプレートのみ | ハンドラは全部生成される。ページはコンポーネントに入力を宣言し、必要なものは `{val}` 束縛で自分で読み込む |
| `+ page.go` の `func Load(w http.ResponseWriter, r *http.Request)` | ハンドラ | 登録だけが生成される。レスポンスは全部実装者のもの |

ハンドラの形でない `Load` は生成エラーになり、いまどうなっていて何でなければ
ならないかを名指しする。かつてはこの2つの間にもう1つ、コンポーネントが描画する
値を返す型付きの `Load` があったが、なくなった。その用途はすべてテンプレート側の
書き方になっている——コンポーネントが必要なものを名指し、自分のローダーを束縛し、
そのローダーのエラーが、何かが書き出される前に応答を決める。
[htmlbind.ja.md](htmlbind.ja.md#応答を選ぶ) を参照。
...
入力の規則は1つ。先頭の引数はルートの動的セグメントで順序もルート順、それ以降は
引数名をキーとするクエリパラメータで、読むのはコンポーネントの引数リストである。
URL はオブジェクトを運ばないので、受け付けるのはスカラーだけである。
...
ページが必要とするものの大半は URL の中にある。だが全部ではない。データベースの
プール、認証済みセッション、リクエストスコープのトレーサ——これらはハンドラの
手前で誰かが載せたリクエストコンテキストに乗って届く。パスセグメントから組み立てた
引数リストでは、どうやっても表現できない。

そのコンテキストに手を伸ばせる形は2つあり、どちらも同じ方法で名乗り出る。Go 側で
引数を宣言するだけだ。テンプレートは何も変わらない。
...
こちらはその場で描画されるので、値がページの引数を経由しない。CSRF トークンや nonce の
ように、必要なページすべてに引数を1つ足すことこそが本当のコストである場合に効く。
宣言側はどちらであっても何も言わない。決めるのは Go のシグネチャであり、関数ごとに
選べる。コンテキストを取らない実装は以前とまったく同じに呼ばれる。

手に入らないのはリクエストそのものである。リクエストが要るならハンドラの段が
要る。そのために用意されている。
