...
コンテキストは URL 入力ではないので、入力として数えない。位置は先頭に限る。Go 自身の
慣習がそうであり、それ以降はルート順の束縛可能なスカラーのままでいられる。先頭以外の
`context.Context` は従来どおりエラーである。その位置に置かれたそれは、実際に URL の値
として差し出されているからだ。
...
どちらの形でも手に入らないのはリクエストそのものである。型付き `Load` に
`*http.Request` を置けば、「トランスポートを持たないこと」が身上のシグネチャに
トランスポートを引き込んでしまう。リクエストが要るならハンドラの段が要る。
そのために用意されている。
