...
//
// It is a conversion rather than a copy, so this declaration and htmlupdate's
// cannot drift: a field added on one side and not the other stops this line
// from compiling, which is the only guard a duplicated struct needs.
func (o Options) core() updatecore.Options { return updatecore.Options(o) }

func fromCore(resp updatecore.Response) Response { return Response(resp) }
...
func (o Options) Negotiate(ctx *fasthttp.RequestCtx) Negotiated {
	return o.core().Negotiate(reader(ctx))
}
