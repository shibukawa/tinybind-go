...
func EmitOpenAPIFragment(pkg, packagePath string, doc Document) ([]byte, error) {
	...
	// Compact JSON: the fragment is baked into the binary as rodata, so
	// indentation would roughly double what every build carries.
	...
	var b bytes.Buffer
	// The opt-out tag lets size-sensitive (TinyGo) builds drop the embedded
	// fragment and its init registration entirely.
	b.WriteString("//go:build !tinybind_no_openapi\n\n")
	b.WriteString("// Code generated by tinybind; DO NOT EDIT.\n\n")
	fmt.Fprintf(&b, "package %s\n\n", pkg)
	b.WriteString("import \"github.com/shibukawa/tinybind-go\"\n\n")
	b.WriteString("func init() {\n")
	fmt.Fprintf(&b, "\thttpbind.RegisterOpenAPIFragmentString(%s, openapiFragmentJSON)\n", strconv.Quote(packagePath))
	b.WriteString("}\n\n")
