...
func emitSelectedFor(plan *PackagePlan, selected map[string]bool, target transportTarget) ([]byte, error) {
	...
	if plan.CBORHTTP {
		if err := checkCBORHTTPTypes(emitted, plan.CBORHTTPProfile); err != nil {
			return nil, err
		}
	}
	var b bytes.Buffer
	...
	for _, t := range emitted {
		if t.Usage&(UsageBind|UsageDecodeJSON) != 0 {
			...
			if plan.CBORHTTP {
				emitCBORHTTPDecode(&b, t, types, plan.CBORHTTPProfile)
			}
		}
		if t.Usage&(UsageWrite|UsageEncodeJSON) != 0 {
			...
			if plan.CBORHTTP {
				emitCBORHTTPEncode(&b, t, types, plan.CBORHTTPProfile)
			}
		}
		if t.Usage&UsageBind != 0 {
			emitBinder(&b, t, types, target, plan.CBORHTTP)
		}
		if t.Usage&UsageWrite != 0 {
			emitWriter(&b, t, types, target, plan.CBORHTTP)
		}
		if t.DirectUsage&UsageScanRows != 0 {
	...
	// it, discovered the way imports are: by what the body actually says.
	if plan.CBORHTTP && bytes.Contains(b.Bytes(), []byte("cborHTTPReadOptions(")) {
		emitCBORHTTPReadOptionsHelper(&b, plan.CBORHTTPProfile)
	}
...
func emitWriter(b *bytes.Buffer, t TypePlan, types map[string]TypePlan, target transportTarget, cborHTTP bool) {
	fmt.Fprintf(b, "func write%s(%s, v %s) error {\n", t.Name, target.writerParams, t.Name)
	if target.writerVar != "r" && !cborHTTP {
		// The request is only there to satisfy the net/http shape; the fasthttp
		// signature has one value and writes through it. The CBOR arm below is
		// the one thing that reads it, for the Accept header.
		b.WriteString("\t_ = r\n")
	}
	if cborHTTP {
		...
		fmt.Fprintf(b, "\t\terr := httpbind.WriteCBORBytes(%s, http.StatusOK, *buf)\n", target.writerVar)
		b.WriteString("\t\tjsonbind.PutBuffer(buf)\n")
		b.WriteString("\t\treturn err\n")
		b.WriteString("\t}\n")
	}
