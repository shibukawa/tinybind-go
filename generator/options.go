...
type Options struct {
	...
	// SQLDialect names the target database for SQL templates: "postgresql" or
	// "mysql". A run that discovers a SQL template must set it. There is no
	// default, because an assumed dialect emits placeholders the target engine
	// rejects, and nothing about the templates reveals the mistake.
	SQLDialect string
